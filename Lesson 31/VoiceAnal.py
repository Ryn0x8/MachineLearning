...
def record_audio(label):
    ...
    wf = wave.open(f"{label.replace(' ', '_')}.wav", 'wb')
    wf.setnchannels(1)
...
def display_stats(stats, text, label):
    ...
    print(f"Estimated Pitch: {stats['pitch']:.2f} Hz")
...
def compare(stats1, stats2):
    ...
    higher_pitch = "Recording 1" if stats1["pitch"] > stats2["pitch"] else "Recording 2"
    pitch_diff = abs(stats1["pitch"] - stats2["pitch"])
    print(f"Higher Pitch: {higher_pitch} by {pitch_diff:.2f} Hz")
